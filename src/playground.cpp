...
void sum_and_add_without_lock() {
    ...
    guarded_thread calc_sum {std::thread {
        [](const decltype(nums) &vec, long long &result){
            long long sum = 0LL;
    ...
    guarded_thread modify_vec {std::thread{
        [](decltype(nums) &vec) {
            for (auto &item: vec) {
                item += 1LL;
            }
...
void sum_and_add_with_lock() {
    ...
    guarded_thread calc_sum {std::thread {
        [&mutex](const decltype(nums) &vec, long long &result){
            long long sum = 0LL;
            {
